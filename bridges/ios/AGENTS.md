...
- Plugging in USB while a Wi-Fi tunnel is running: the service builds a second tunnel for the same device, but the registry keeps reporting the older one, so `tunnel-status` shows one active interface and one unassociated. WDA then fails on both transports, and `IOS_CUCTL_TRANSPORT=usb` does not help because the registry entry is what the bridge follows. Restart the service with the cable already attached, which is the only reliable way to get a single USB-backed tunnel: `sudo systemctl restart "ios-computer-use-tunneld@$(id -un).service"`. Every WDA session that succeeded during testing followed that exact order.
- WDA fails to launch while screenshots still work: check `tunnel-status` for `interfaces.unassociated`. Cancelled tunnels can leave persistent TUN devices behind, and each keeps a ULA `/64` route. Host-to-device traffic (screenshots, `apps`, `press`, `activate`) still routes correctly, but WebDriverAgent needs the device to connect back to the host, and that callback lands on the wrong interface. The device log shows `Exiting due to IDE disconnection` and the host shows `Connection closed while waiting for proxied service`. The tunneld service now deletes each interface it rebuilds; clear anything left over from an older build with `sudo systemctl restart "ios-computer-use-tunneld@$(id -un).service"`. Raising `wdaLaunchTimeout` does not help, because the link is broken rather than slow.
